...
from src.models.entities.Hydrography import Well, Water_Source, Dam, param_demandas
...
def start(apiflask):
    ...
    ns = apiflask.namespace(
        "Hydrographies", description="CHAMBO API")
    current_folder = os.path.dirname(os.path.realpath(__file__))
    f = open(os.path.join(current_folder, "config.json"))
    config = json.load(f)


    @ns.route("/sub-catchments-MELCA", methods=["GET"])
    class SubCatchments(Resource):
        def get(self):
            subcatchments = HydrographyModel.get_sub_catchments_MELCA()
            return jsonify(subcatchments)
    
    @ns.route("/sub-catchment-MELCA/<id>")
    class SubCatchment(Resource):
        def get(self,id):
            subcatchment = HydrographyModel.get_sub_catchment_MELCA(int(id))
            return jsonify(subcatchment)

    @ns.route("/sub-catchments-MELCA/<id>/upper-sub-catchments-MELCA")
    class upperSubCatchment(Resource):
        def get(self,id):
            subcatchment = HydrographyModel.get_upper_sub_catchments(int(id))
            return jsonify(subcatchment)

    @ns.route("/sub-catchments-MELCA/<id>/sub-catchment-MELCA-navigation")
...
#_________________________________________________________________________
    ...
    @ns.route("/parametros-usos-demandas/subcatchment-MELCA/<Sub_Catchment_MELCA_id>")
    class demanda_MELCA(Resource):
        def get(self,Sub_Catchment_MELCA_id):
            eco = HydrographyModel.get_usos_demands_MELCA(int(Sub_Catchment_MELCA_id))
            return jsonify(eco)   

    @ns.route("/parametros-ret-demandas/subcatchment-MELCA/<Sub_Catchment_MELCA_id>")
    class demanda_MELCA(Resource):
        def get(self,Sub_Catchment_MELCA_id):
            eco = HydrographyModel.get_ret_demands_MELCA(int(Sub_Catchment_MELCA_id))
            return jsonify(eco)   

    Add_demanda_params = apiflask.model(
                                        "demandas_params",
                                        {
                                            "id": fields.Integer,
                                            "inic": fields.Integer,
    ...
    @ns.route("/parametros-usos-demandas/demanda-add")
    class Adddemanda(Resource):
        ...
        def post(self):
            ...
            subcatchment =  param_demandas(id,  inic,fin,tipo, qmax, tret)
    ...
    dell_demanda_params = apiflask.model(
                                        "dell_demanda_params",
                                        {
                                            "id": fields.String
    ...
    @ns.route('/parametros-usos-demandas/demanda-delete', methods=['DELETE'])
    class DELdams(Resource):
        ...
        def delete(self):            
            try:
                ...
                if affected_rows == 1:
                    return jsonify({'message': "demanda deleted"})
                else:
                    return jsonify({'message': "No demanda deleted"})
