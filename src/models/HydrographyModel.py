...
from .entities.Hydrography import  descriptores_chambo, Navigation, param_usos_1  #,Well, Water_Source, Parametros_LEM, Param_Chambo,Dam,ecosystems,irrigations,mining_centers,potable_water_demands,param_demandas,param_demandas_tipos,param_demandas_info,Navigation,param_ret
import pandas as pd
import os
...
class HydrographyModel():
    #descriptores chambo
    @classmethod    
    def get_sub_catchments(self):        
        try:
            connection = get_connection()
            subcatchments = []
            with connection.cursor() as cursor:
                cursor.execute("SELECT * FROM descriptores_chambo ORDER BY id ASC")
                resultset = cursor.fetchall()                
                for row in resultset:
                    subcatchment = descriptores_chambo(row[0], row[1], row[2], row[3],row[4], row[5], row[6], row[7], row[8])
    ...
    @classmethod
    def get_sub_catchment(self, id):
        try:
            connection = get_connection()
            with connection.cursor() as cursor:
                cursor.execute("SELECT * FROM descriptores_chambo WHERE id = %s", (id,))
                row = cursor.fetchone()
                subcatchment = None
                if row != None:
                    subcatchment = descriptores_chambo(row[0], row[1], row[2], row[3],row[4], row[5], row[6], row[7], row[8] )
    ...
    def get_upper_sub_catchments(self,id):        
        try:
            ...
            subcatchments = []
            with connection.cursor() as cursor:
                cursor.execute('SELECT * FROM descriptores_chambo WHERE "Drena_a" = %s', (id,))
                resultset = cursor.fetchall()                
                for row in resultset:
                    subcatchment = descriptores_chambo(row[0], row[1], row[2], row[3],row[4], row[5], row[6], row[7], row[8])
                    subcatchments.append(subcatchment.to_JSON())
    ...
    def get_navigation(self,id):        
        try:
            connection = get_connection()
            with connection.cursor() as cursor:
                cursor.execute('SELECT id FROM descriptores_chambo WHERE "Drena_a" = %s', (id,))
                resultset = cursor.fetchall()
                resultset = [resultset[i][0] for i in range(0,len(resultset))]
                cursor.execute('SELECT "Drena_a" FROM descriptores_chambo WHERE id = %s', (id,))
...
# #_________________________________________________________________________
    ...
    def get_params_usos_demandas(self):
        ...
        try:
            ...
            subcatchments = []
            with connection.cursor() as cursor:
                cursor.execute('SELECT * FROM param_usos_1 ORDER BY "ID" ASC')
                resultset = cursor.fetchall()                
                for row in resultset:
                    subcatchment = param_usos_1(row[0], row[1], row[2], row[3],row[4], row[5])
                    subcatchments.append(subcatchment.to_JSON())
            connection.close()
            return subcatchments
        except Exception as ex:
    ...
    def get_params_usos_demanda(self, id):
        try:
            connection = get_connection()
            with connection.cursor() as cursor:
                cursor.execute('SELECT * FROM param_usos_1 WHERE "ID" = %s', (id,))
                row = cursor.fetchone()
                subcatchment = None
                if row != None:
                    subcatchment = param_usos_1(row[0], row[1], row[2], row[3],row[4], row[5])
    ...
    @classmethod
    def get_usos_demands_sub_catchemnt(self,Sub_Catchment_MELCA_id):
        try:
            connection = get_connection()
            wells = []
            with connection.cursor() as cursor:               
                cursor.execute('SELECT * FROM param_usos_1 WHERE "INIC" = %s', (Sub_Catchment_MELCA_id,))                
                resultset = cursor.fetchall()
                for row in resultset:
                    well = param_usos_1(row[0], row[1], row[2], row[3],row[4],row[5])
    ...
    @classmethod
    def get_ret_demands_sub_catchemnt(self,Sub_Catchment_MELCA_id):
        try:
            connection = get_connection()
            wells = []
            with connection.cursor() as cursor:               
                cursor.execute('SELECT * FROM param_usos_1 WHERE "FIN" = %s', (Sub_Catchment_MELCA_id,))                
                resultset = cursor.fetchall()
                for row in resultset:
                    well = param_usos_1(row[0], row[1], row[2], row[3],row[4],row[5])
                    wells.append(well.to_JSON())
    ...
    def add_demanda(self, well):        
        try:
            connection = get_connection()            
            with connection.cursor() as cursor:                
                cursor.execute("""INSERT INTO param_usos_1 ("ID", "INIC", "FIN", "TIPO", "QMAX", "TRET") 
    ...
    def delete_demanda(self, id):
        try:
            connection = get_connection()
            with connection.cursor() as cursor:
                cursor.execute('DELETE FROM param_usos_1 WHERE "ID" = %s', (id,))
                affected_rows = cursor.rowcount
                connection.commit()
            connection.close()            
            return affected_rows
